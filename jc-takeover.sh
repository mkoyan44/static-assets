...
echo "Please write your current user name, it was obtain with id -an command on previus step?"
read oldUser

echo "Write your JC username, example for jsmit@example.com account the username IS jsmit?"
read newUser

if [ -z "$oldUser" ]
then
	echo "${log} Error: $oldUser required" 2>&1 | tee -a JC_RENAME.log
	exit 1
fi

if [ -z "$newUser" ]
then
	echo "${log} Error: $newUser required" 2>&1 | tee -a JC_RENAME.log
	exit 1
fi
